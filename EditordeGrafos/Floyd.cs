...
namespace EditordeGrafos
{
    public partial class Floyd : Form
    {
        ...
        public Floyd(Graph g)
        {
            ...
            //richTextBox_Floyd.AppendText("Matriz costos  inicial: \n");
            MatrizCostos();
            for (int i = 0; i < graph.Count; i++)
            {
                for (int j = 0; j < graph.Count; j++)
                {
                    costoInicial_Floyd.AppendText(costos[i, j] + "\t");
                }
                costoInicial_Floyd.AppendText("\n");
            }
            ...
            for (int i = 0; i < graph.Count; i++)
            {
                for (int j = 0; j < graph.Count; j++)
                {
                    costoMinimo_Floyd.AppendText(costosMinimo[i, j] + "\t");
                }
                costoMinimo_Floyd.AppendText("\n");
            }
        }
        ...
        private void ImprimeResultados(string[] D, string path, string origenName)
        {
            ...
            costoTotal = D[graph.IndexOf(destino)];
        ...
        private void DijkstraIniciate_Click(object sender, EventArgs e)
        {
            Dijkstra(origen, destino);
        }
        ...
        public void MatrizCostosMinimo()
        {
            int nodes;
            nodes = graph.Count;
            costosMinimo = new string[nodes, nodes];
            int i = 0;
            int j = 0;

            foreach (NodeP origenCM in graph)
            {
                j = 0;
                foreach (NodeP destinoCM in graph)
                {
                    costosMinimo[i,j] = Floyd2(origenCM, destinoCM);
                    j++;
                }
                i++;
            }
        ...
        public string Floyd2(NodeP origenCM, NodeP destinoCM)
        {
            string minimo;
            List<NodeP> V = new List<NodeP>();
            List<NodeP> S = new List<NodeP>();
            ...
            foreach (NodeP p in graph)
            {
                ...
                if (p != origenCM)
                {
                    D[graph.IndexOf(p)] = costos[graph.IndexOf(origenCM), graph.IndexOf(p)];
                    if (D[graph.IndexOf(p)] != "x")
                    {
                        R[graph.IndexOf(p)] = graph[graph.IndexOf(origenCM)];
                    }
                }
                else
                {
            ...
            if (R[graph.IndexOf(destinoCM)] != null)
            {
                path = consigueCamino(R, destinoCM, origenCM, destinoCM, "");
            }
            else
            {
                path = "ERROR: NO HAY CAMINOS DIRIGIDOS";
            }
            minimo = regresaPesoMinimo(D, path, origenCM.Name, destinoCM);
            return minimo;
        }

        private string regresaPesoMinimo(string[] D, string path, string origenNameCM, NodeP destinoCM)
        {
            string vexSol = "D[" + origenNameCM + "] = {";
            string costoTotal;
            ...
            costoTotal = D[graph.IndexOf(destinoCM)];
            return costoTotal;
        }
    }
}
